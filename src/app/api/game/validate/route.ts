...
export async function POST(req: NextRequest) {
  ...
  const team = data.teams.find((t: { teamId: string; [key: string]: unknown }) => t.teamId === teamId);
  const puzzle = data.puzzles.find((p: { _id: string; [key: string]: unknown }) => p._id === puzzleId);
